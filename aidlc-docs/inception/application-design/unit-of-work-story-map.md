...
## U6 — 프로필·자격매칭·추천 (E6, v2) — 7 stories
| Story | 제목 | 우선순위 |
|---|---|---|
| US-6.1 | 가구 프로필 입력/저장 | Must |
| US-6.2 | 프로필 변경 시 추천 재계산 | Must |
| US-6.3 | 공고 자격 판정 | Must |
| US-6.4 | 자격 기준표(config) 관리 | Should |
| US-6.5 | 추천 점수·정렬 | Must |
| US-6.6 | 추천 사유 | Should |
| US-6.7 | 신규 추천 Push | Should |

## 커버리지 검증
- **총 스토리(v2)**: 31 = U1(7) + U2(3) + U3(6) + U4(4) + U5(4) + U6(7) ✅
- (구) 총 스토리: 24 = U1(7) + U2(3) + U3(6) + U4(4) + U5(4) ✅
- **미배정 스토리**: 없음 ✅
