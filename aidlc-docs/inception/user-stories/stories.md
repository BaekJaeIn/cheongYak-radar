...
## Epic E6 — 프로필 · 자격매칭 · 추천 (v2 추가, FR-8~10) — 7 stories

> 방향 전환(우리 부부 전용 추천). 페르소나 P1을 **단일 가구(나+여자친구)** 로 구체화. 신규 단위 **U6**.

### US-6.1 가구 프로필 입력/저장 (v2 · Must · P1)
**As** 우리 부부(P1), **I want** 소득·거주지·혼인·청약통장·자산·무주택·생애최초·희망조건을 입력·저장하고 싶다 **so that** 자격·추천이 우리 기준으로 계산된다.
- **AC1** — Given `/settings`(프로필)에서 값을 입력하면, When 저장하면, Then 가구 프로필 1건이 저장되고 이후 추천에 사용된다.
- **AC2** — Given 모르는 항목(예: 혼인 예정일), Then 비워둘 수 있고 매칭은 가정/기준표로 동작한다. (O-1~O-5)

### US-6.2 프로필 변경 시 추천 재계산 (v2 · Must · P1)
**As** P1, **I want** 프로필(여친 소득 등)을 바꾸면 추천이 다시 계산되길 원한다.
- **AC1** — Given 프로필을 갱신하면, When 추천 화면을 열면, Then 변경된 기준으로 자격/점수가 재계산된다.

### US-6.3 공고 자격 판정 (v2 · Must · P1)
**As** P1, **I want** 각 공고에 대해 우리가 신청 가능한 공급유형을 알고 싶다.
- **AC1** — Given 공고와 프로필이 있을 때, Then 공급유형별(신혼특공·신혼희망타운·생애최초·일반) 신청 가능 여부가 판정된다.
- **AC2** — Given 부적합하면, Then 사유(소득초과/거주미달/통장 등)가 표시된다. (FR-9.4)
- **AC3** — Given 소득·자산 한도는, Then 연도별 기준표(config)로 평가된다. (FR-9.3)

### US-6.4 자격 기준표(config) 관리 (v2 · Should · P2)
**As** 운영자(P2), **I want** 도시근로자 소득·자산 한도 등 기준값을 설정으로 관리하고 싶다 **so that** 법령 변경에 대응한다.
- **AC1** — Given 연도별 기준표 파일이 있을 때, Then 매칭이 그 값을 참조한다.

### US-6.5 추천 점수·정렬 (v2 · Must · P1)
**As** P1, **I want** 신청 가능한 공고를 우리에게 유리한 순으로 보고 싶다.
- **AC1** — Given 자격 통과 공고들에 대해, Then 청약순위·소득여유·거주가점·희망지역·면적적합·마감임박 가중으로 점수화되어 정렬된다. (FR-10.1)
- **AC2** — Given 관심지역(안양·군포·광명·서울 서남권)일 때, Then 가산점을 받고, 그 외 서울·경기도 노출된다. (A4=A)

### US-6.6 추천 사유 (v2 · Should · P1)
**As** P1, **I want** 왜 추천/부적합인지 한눈에 알고 싶다.
- **AC1** — Given 추천 공고일 때, Then "왜 유리한지" 요약이 표시된다(Claude 재사용 가능). (FR-10.4)

### US-6.7 신규 추천 Push (v2 · Should · P1) — US-5.4 확장
**As** P1, **I want** 새로 우리 자격에 맞는 공고가 뜨면 알림받고 싶다.
- **AC1** — Given 수집 후 새로 자격 충족된 공고가 생기면, Then Web Push로 알린다. (FR-10.3)

## v2 개정 노트 (기존 스토리 영향)
- **지역**: 전 스토리 대상 지역을 **서울·경기로 한정** (C-6). 수집/추천 모두.
- **US-3.1~3.6 (목록·필터)**: 목록 화면을 **추천 피드**(점수순)로 재해석. 필터는 보조. `/settings`는 **프로필 입력** 중심으로 변경.
- **US-4.3 (자격요약)**: 자격 판정 결과(US-6.3)와 통합 표시.
- **US-5.4 (Push)**: 신규공고 → **신규 추천** 기준으로 발송(US-6.7).
- **페르소나**: P1 = 단일 가구(예비신혼, 부천 거주, 무주택). P2(운영) 유지.
...
**총 31개 스토리** · 6 Epic (E1~E6) · 2 페르소나 — v2 반영(E6 +7)
