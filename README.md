...
- **캘린더 추가** — 공고 상세에서 청약시작일을 Google 캘린더 종일 일정으로 추가 (삼성캘린더 등 계정 동기화 캘린더에 표시, 계정 연동 불필요)
- **공고분석 (PDF 자격판정)** — 원하는 공고 PDF를 업로드하면 Gemini가 자격 기준을 추출하고, 가구 프로필 기준으로 공급유형별 지원 가능/조건부/불가를 판정 (프로필은 LLM에 전송되지 않음)
...
│   ├── notice/[id]/         # 공고 상세
│   ├── bookmarks/           # 관심 공고
│   ├── analyze/             # 공고분석 (PDF 자격판정)
│   ├── settings/            # 프로필/필터 설정
│   └── api/
│       ├── subscribe/       # 웹 푸시 구독 저장
│       ├── profile/         # 가구 프로필 저장
│       └── analyze/         # 공고 PDF 자격판정 프록시
└── features/                # 기능별 모듈 (feed, detail, recommendations, notices,
                             #   profile, bookmarks, analyze, notifications, pwa, nav)
...
│   ├── analyze.ts           # 공고 PDF 자격판정 (Gemini 추출 → 매칭)
